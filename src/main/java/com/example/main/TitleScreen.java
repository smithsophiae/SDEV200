//TitleScreen.java opens title screen and changes scene to main (game) scene when play button is clicked
    ...
    public Scene createTitleScreen(Stage primaryStage) {
        ...
        try {
            pixelFont = Font.loadFont(
                    getClass().getResource("/images/PixelifySans-VariableFont_wght.ttf").toExternalForm(), 20);
        } catch (Exception e) {
            System.out.println("ERROR: Font file not found!");
        }

        // If font is null, use fallback
        if (pixelFont == null) {
            System.out.println("ERROR: Font failed to load! Using default font.");
            pixelFont = Font.font("Arial", 20);
        }
        // Title Screen layout
        ...
        Image titleBackgroundImage = new Image("/images/title3.jpg");
        ...
        playButton.setOnAction(event -> {
            ...
            // Create a SwingNode to embed the JPanel
            javafx.embed.swing.SwingNode swingNode = new javafx.embed.swing.SwingNode();
            swingNode.setContent(gamePanel); // Embeds the GamePanel (JPanel) into SwingNode

            // Create a StackPane to hold the SwingNode
            javafx.scene.layout.StackPane stackPane = new javafx.scene.layout.StackPane();
            stackPane.getChildren().add(swingNode);

            // Create the JavaFX Scene with the StackPane
            Scene gameScene = new Scene(stackPane, gamePanel.screenWidth, gamePanel.screenHeight);
